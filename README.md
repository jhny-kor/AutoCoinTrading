...
- 전략 공통값은 `config/runtime.toml` 기준으로 관리하도록 정리했습니다.
- 코인별 이격도, 익절률, 손절률을 다르게 설정할 수 있도록 정리했습니다.
- 심볼별 최소 주문 수량도 `config/runtime.toml` 기준으로 다르게 관리할 수 있도록 정리했습니다.
- 알트 감시 심볼 목록도 `config/runtime.toml` 기준으로 관리해 종목 추가 시 코드 수정 범위를 줄였습니다.
...
- 재시작 후 평균 진입가를 복구하지 못한 포지션은 현재가로 임시 진입가를 만들지 않고 자동 매매를 보류하도록 바꿨습니다.
- OKX 주문 실패는 `order_failed` 경고만 남기고 `loop_error` 로 다시 증폭하지 않도록 정리했습니다.
...
- `strategy_settings.py`: `config/runtime.toml` + override 레이어 기준 공통 전략 값을 읽는 모듈
...
- `config/runtime.toml`의 `okx.alt_symbols`, `upbit.alt_symbols` 기준으로 감시할 알트 심볼을 관리합니다.
- 텔레그램 `/positions`, `/analysis`, 분석 수집기는 위 알트 목록에 BTC 전용 심볼을 자동으로 합쳐서 사용합니다.
- 운영 알트 외에 추가 관찰만 하고 싶은 심볼은 `config/runtime.toml`의 `analysis.okx_symbols`, `analysis.upbit_symbols` 기준으로 관리합니다.
- 심볼별 이격도, 익절률, 손절률, 최소 주문 수량은 `config/runtime.toml`의 strategy 설정 기준으로 함께 관리하는 것이 안전합니다.
...
- `config/runtime.toml` 또는 `config/runtime.local.toml`에서 `telegram.enabled=true` 로 켭니다.
- `.env.secrets`의 `TELEGRAM_BOT_TOKEN` 에 BotFather 에서 받은 토큰을 넣습니다.
- `.env.secrets`의 `TELEGRAM_CHAT_ID` 에 메시지를 받을 chat id 를 넣습니다.
...
텔레그램 명령 리스너가 실행 중이면 `config/runtime.toml` + override 설정에 따라 일일 리포트도 자동으로 전송됩니다.
...
텔레그램 명령 리스너가 실행 중이면 `config/runtime.toml` + override 설정에 따라 최근 7일 기준 주간 리포트도 자동으로 전송됩니다.
...
- 알트 봇은 `config/runtime.toml`의 알트 심볼 목록 기준으로 여러 종목을 순회하며, 최근 로그 기준으로 일부 종목의 거래량 필터와 이격도 기준을 완화해 테스트 중입니다.
- 앞으로는 현재 전략을 충분히 관찰한 뒤 코인별 필터 값을 더 세밀하게 조정하고, 필요하면 RSI나 거래 횟수 제한 같은 보조 필터를 추가 검토할 수 있습니다.

## 현재 역할 분리

- 기존 OKX 봇: `config/runtime.toml`의 `okx.alt_symbols`
- 기존 업비트 봇: `config/runtime.toml`의 `upbit.alt_symbols`
