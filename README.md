...
현재는 기본 OHLCV 외에도 거래량 배수, 변동성, RSI, 최근 범위 위치, 상위 타임프레임 상태, 호가 스프레드, 공개 기준 필터 통과 여부까지 함께 저장합니다.
...
알림은 `ma_crossover_bot.py` 와 `upbit_ma_crossover_bot.py` 실행 중 자동으로 전송됩니다.
...
- BTC 전용 봇은 ATR 기반 손절과 익절 구간 진입 후 전량 트레일링 청산 구조를 함께 테스트 중입니다.
...
이 전략은 `5분봉/15분봉`, `EMA`, `거래량 확인`, `ATR 기반 변동성 필터`, `물타기 없음`, `ATR 또는 최근 스윙 기반 손절`, `익절 구간 도달 후 전량 트레일링 청산` 구조를 사용합니다.
