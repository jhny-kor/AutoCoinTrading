...
def build_regime_change_message(
    *,
    ...
    return (
        f"{exchange_name.upper()} {symbol} 레짐 변경\\n"
        f"{previous_text} -> {snapshot.regime}\\n"
        f"거래량 배수 {volume_text} | 평균 절대 변화율 {abs_text} | RSI {rsi_text} | "
        f"공개 준비 {snapshot.public_buy_ready}"
    )
