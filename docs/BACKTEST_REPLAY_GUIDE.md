...
.venv/bin/python backtest_replay.py run \
  ...
  --btc-reference-input data/upbit_btc_krw_1m.jsonl \
  --timeframe 1m \
  --initial-cash 1000000
```

`--btc-reference-input` 을 주면 알트 리플레이도 실거래처럼 BTC 상관관계 필터를 함께 재생합니다.
...
- `--btc-reference-input`
  - 알트 전략에서 BTC 상관관계 필터를 함께 재생할 때 사용할 BTC 기준 OHLCV 파일
- `--initial-cash`
  - 시작 자금
- `--fee-rate-pct`
  - 편도 수수료율 직접 지정
