"""
수정 요약
- 2026-06-12: 알트 순익 trailing exit 정책을 OKX 알트 루프에 연결했다.
...
- BTC LOW_ENERGY 축소 스케일에 심볼별 override 를 적용해 ETH/USDT 는 더 보수적으로, XRP/USDT 는 덜 보수적으로 진입 비중을 줄이도록 조정했다.
    ...
    while True:
        ...
        for m in markets:
            ...
            try:
                ...
                latest_symbol_record = load_latest_symbol_record(exchange_name="okx", symbol=symbol)
                ...
                estimated_sell_ratio = (
                    1.0
                    if sol_probe_time_exit_triggered or alt_profit_trailing_exit_triggered
                ...
                if entry_ready:
                    if usdt_to_use <= strategy.min_buy_order_value:
                        ...
                        buy_fill_state = apply_alt_buy_fill_state(
                            symbol=symbol,
                            bought_amount=estimated_bought_amount,
                            last_close=last_close,
                            has_position=has_position,
                            avg_entry_price=avg_entry_price,
                            base_free=base_free,
                        ...
                        log(
                            f"[{symbol}] 갱신된 평균 진입가: {buy_fill_state.entry_price_after:.4f}"
                        )
                ...
                # 매도 신호 발생 시, 분할 청산 + 최소 익절률 조건을 만족하면 청산
                    ...
                    sell_order_plan = resolve_alt_sell_order_by_min_amount(
                        symbol=symbol,
                        base=base,
                        amount=amount,
