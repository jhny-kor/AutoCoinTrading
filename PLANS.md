...
- BTC 전용 전략
  ...
  - 청산 조건: ATR 또는 최근 스윙 기반 손절, 익절 구간 진입 후 최고가 대비 되돌림 전량 트레일링 청산, 수수료 반영 순익 보호 익절, 선택적 데드크로스 추세 종료 청산
  - 특징: 보수적 1회 추가매수 허용, 약한 추세 구간 진입 축소, 최소 주문 수량 선제 차단
- 알트 전용 전략
...
- 최근 조정으로 알트는 `거래량 필터 완화`, `이격도 완화`를 먼저 적용했고, BTC는 `익절 구간 도달 후 전량 트레일링 청산`, `약한 추세 구간 진입 축소`, `수수료 반영 순익 보호 익절`을 추가했습니다.
